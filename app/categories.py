...
GROUPS: tuple[Group, ...] = (
    ...
    Group(MATERIAL, "พัสดุ", frozenset({"4", "5", "6", "8"})),
    # แยกออกจาก "พัสดุ" เมื่อ 17 ก.ย. 2569 — เป็นของลงทุน ไม่ใช่ของใช้สิ้นเปลือง
    # ถ้ารวมไว้ แผนกที่ซื้อเครื่องมือหนึ่งเครื่องจะดูเหมือนใช้ของเปลืองกว่าแผนกอื่นทั้งปี
    # หลักฐาน (ยอดเบิก 12 เดือนถึง ก.ย. 2569): ราคาต่อหน่วยกลางของหมวด 7 คือ 34,500 บาท
    # เทียบกับหมวด 4 ที่ 2,200 และหมวด 6 ที่ 170 · 101 จาก 285 รหัสราคาเกินแสนบาทต่อหน่วย
    # · หน่วยนับที่พบบ่อยที่สุดคือ EG (เครื่อง) · ชื่อเป็นเครื่องมือล้วน เช่น เครื่องเอกซเรย์
    # สวนหัวใจ 37.98 ล้าน หุ่นยนต์ช่วยผ่าตัดข้อเข่า 18 ล้าน เครื่องสูบน้ำดับเพลิง 5.69 ล้าน
    # **ยังไม่ได้ให้เจ้าหน้าที่พัสดุยืนยันว่าหมวด 7 = ครุภัณฑ์ ตรงตามที่โรงพยาบาลใช้จริง**
    Group(EQUIPMENT, "ครุภัณฑ์", frozenset({"7"})),
    ...
    # แยกเมื่อ 17 ก.ย. 2569 — ผู้ใช้สั่งให้ดูงานจ้างได้ด้วย และเป็นยอดใหญ่อันดับสามของการซื้อ
    # หลักฐาน: ใบรับ 12 เดือนของคลังพัสดุหมวด 9 = 356 ล้านบาท เลขใบรับ RA68-… ใบสั่ง PA47-…
    # (PA6904-066 คือบันทึกขอจ้างซ่อมใน docs/contract_work.md) · ชื่อรายการตัวอย่าง
    # "จ้างถ่ายเอกสารทั่วไป" · ห้องชันสูตรหมวด 9 เป็นรายการส่งตรวจภายนอก (LRA68-…)
    # งานจ้างไม่มีคงคลังและไม่ถูกเบิก จึงเห็นได้จากใบรับเท่านั้น
    Group(HIRE, "งานจ้างและบริการ", frozenset({"9"})),
    Group(OTHER, "อื่น ๆ", frozenset()),  # ทุกหมวดที่เหลือ เช่น 13 (รายการทดสอบราคา 1 รหัส)
)

#: กลุ่มที่เป็นของใช้สิ้นเปลือง — คิดเดือนคงคลังได้ ส่วนครุภัณฑ์และงานจ้างคิดไม่ได้ เพราะไม่ได้ "ใช้หมด"
CONSUMABLE_GROUPS = frozenset({DRUG, MEDICAL_SUPPLY, MATERIAL, FOOD})
